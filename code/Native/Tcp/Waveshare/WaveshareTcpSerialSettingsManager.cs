...
namespace RJCP.IO.Ports.Native.Tcp.Waveshare
{
    public class WaveshareTcpSerialSettingsManager : ITcpSerialPortSettingsManager
    {
        ...
        {
            if (settings is WaveshareSerialPortSettings waveshareSettings)
            {
                var credentials = Convert.ToBase64String(Encoding.ASCII.GetBytes(m_Username + ":" + m_Password));
                var parity = ((int) waveshareSettings.Parity) + 1;
                var flowMode = GetFlowModeFromHandshake(waveshareSettings.Handshake);

                var url = $"http://{m_Host}/config.cgi" +
                          $"?br={waveshareSettings.BaudRate}" + // baudrate
                          $"&bc={waveshareSettings.DataBits}" + // data bits
                          $"&parity={parity}" + // parity
                          $"&stop={waveshareSettings.StopBits}" + // stop bits
                          "&flow=1" + // Serial mode (1=RS232; 5=RS485; 6=RS422; 8=Dial-Switch)
                          $"&xon={flowMode}" + // flow control (0=NONE; 3=RTS/CTS; 32=XON/XOFF)
                          $"&tim={waveshareSettings.UartPacketTime}" + // UART Packet time
                          $"&num={waveshareSettings.UartPacketLength}" + // UART Packet length
                          $"&srf={(waveshareSettings.SyncBaudRate ? 1 : 0)}" + // sync baud rate
